-- Migration 093: Referência geográfica oficial do Brasil (IBGE)
-- Fonte: API de Localidades do IBGE, consultada em 2026-09-21.
-- Contém 27 UFs e 5.571 municípios/distrito federal segundo a resposta oficial atual.
...
CREATE TABLE IF NOT EXISTS public.brazilian_cities (
  ibge_code INTEGER PRIMARY KEY,
  state_ibge_code INTEGER NOT NULL REFERENCES public.brazilian_states(ibge_code) ON UPDATE CASCADE ON DELETE RESTRICT,
  name TEXT NOT NULL,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  CONSTRAINT uq_brazilian_cities_state_name UNIQUE (state_ibge_code, name)
);

CREATE INDEX IF NOT EXISTS idx_brazilian_cities_state_name ON public.brazilian_cities(state_ibge_code, name);
...
DROP POLICY IF EXISTS "Brazilian states are publicly readable" ON public.brazilian_states;
CREATE POLICY "Brazilian states are publicly readable" ON public.brazilian_states FOR SELECT TO anon, authenticated USING (true);
DROP POLICY IF EXISTS "Brazilian cities are publicly readable" ON public.brazilian_cities;
CREATE POLICY "Brazilian cities are publicly readable" ON public.brazilian_cities FOR SELECT TO anon, authenticated USING (true);

INSERT INTO public.brazilian_states (ibge_code, uf, name, region_code, region_name) VALUES
  (12, 'AC', 'Acre', 1, 'Norte'),
  ...
  (32, 'ES', 'Espírito Santo', 3, 'Sudeste'),
...
ON CONFLICT (ibge_code) DO UPDATE SET uf = EXCLUDED.uf, name = EXCLUDED.name, region_code = EXCLUDED.region_code, region_name = EXCLUDED.region_name, updated_at = NOW();

INSERT INTO public.brazilian_cities (ibge_code, state_ibge_code, name) VALUES
...
ON CONFLICT (ibge_code) DO UPDATE SET state_ibge_code = EXCLUDED.state_ibge_code, name = EXCLUDED.name, updated_at = NOW();
...
ALTER TABLE public.business_locations ADD COLUMN IF NOT EXISTS city_ibge_code INTEGER;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'business_locations_city_ibge_code_fkey') THEN
    ALTER TABLE public.business_locations ADD CONSTRAINT business_locations_city_ibge_code_fkey FOREIGN KEY (city_ibge_code) REFERENCES public.brazilian_cities(ibge_code) ON UPDATE CASCADE ON DELETE RESTRICT;
  END IF;
END $$;
CREATE INDEX IF NOT EXISTS idx_business_locations_city_ibge_code ON public.business_locations(city_ibge_code);

-- Backfill conservador: somente nome oficial + UF exatos, ignorando abreviações ambíguas.
UPDATE public.business_locations bl
SET city_ibge_code = c.ibge_code
FROM public.brazilian_cities c
JOIN public.brazilian_states s ON s.ibge_code = c.state_ibge_code
WHERE bl.city_ibge_code IS NULL
  AND LOWER(TRIM(bl.city)) = LOWER(c.name)
  AND UPPER(TRIM(bl.state)) = s.uf;

CREATE OR REPLACE FUNCTION public.sync_business_location_from_ibge_city()
RETURNS TRIGGER LANGUAGE plpgsql SET search_path = '' AS $$
DECLARE v_city RECORD;
BEGIN
  IF NEW.city_ibge_code IS NOT NULL THEN
    SELECT c.name AS city_name, s.uf INTO v_city FROM public.brazilian_cities c JOIN public.brazilian_states s ON s.ibge_code = c.state_ibge_code WHERE c.ibge_code = NEW.city_ibge_code;
    IF NOT FOUND THEN RAISE EXCEPTION 'Invalid IBGE city code: %', NEW.city_ibge_code; END IF;
    NEW.city := v_city.city_name; NEW.state := v_city.uf;
  END IF;
  RETURN NEW;
END; $$;
DROP TRIGGER IF EXISTS trg_sync_business_location_from_ibge_city ON public.business_locations;
CREATE TRIGGER trg_sync_business_location_from_ibge_city BEFORE INSERT OR UPDATE OF city_ibge_code ON public.business_locations FOR EACH ROW EXECUTE FUNCTION public.sync_business_location_from_ibge_city();

GRANT SELECT ON public.brazilian_states, public.brazilian_cities TO anon, authenticated;
