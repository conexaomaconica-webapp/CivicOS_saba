-- ============================================================================
-- Migration 090: Expand available_cities in public_directory_home_data
-- Lists ONLY cities from published active commercial businesses (via business_locations).
-- Excludes organizations / Masonic Lodges from the commercial directory filter.
-- ============================================================================
...
BEGIN
  ...
  -- 5. Available Cities (Cidades únicas de empresas comerciais ativas publicadas)
  -- NOTA: Exclusivamente obtidas de businesses + business_locations (sem organizações/Lojas Maçônicas)
  SELECT COALESCE(jsonb_agg(city_name ORDER BY city_name ASC), '[]'::jsonb)
  INTO v_cities
  FROM (
    SELECT DISTINCT TRIM(bl.city) AS city_name
    FROM public.business_locations bl
