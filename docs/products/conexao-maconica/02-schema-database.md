...
As tabelas de extensão abaixo modelam os vínculos comerciais garantindo integridade composta multi-tenant `(tenant_id, id)` e preservação histórica de dados de auditoria (evitando apagamento em cascata de atores via `ON DELETE SET NULL`):
...
CREATE UNIQUE INDEX uq_bml_primary_per_business ON public.business_masonic_links (tenant_id, business_id) WHERE (is_primary = true AND status = 'active');
```
...
##### 6.15.3 `public.business_masonic_link_authorizations` (Registro Empresarial Auditável)
```sql
...
##### 6.15.4 `public.business_masonic_link_publication_consents` (Consentimento Granular LGPD)
```sql
...
##### 6.15.5 `public.business_masonic_link_contests` (Entidade Formal de Contestação)
```sql
...
##### 6.15.6 `public.business_masonic_link_history` (Audit Trail Imutável Ampliado)
```sql
...
#### 6.16 Diretrizes de Segurança, Armazenamento e Privacidade para Evidências

1. **Armazenamento Privado**: Os arquivos de evidência são gravados em storage bucket privado com criptografia em repouso garantida pelo provedor de infraestrutura.
