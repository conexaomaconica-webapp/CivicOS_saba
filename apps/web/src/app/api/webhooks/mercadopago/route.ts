...
import { MercadoPagoBillingAdapter } from '@/lib/billing/billing-adapters';
...
export async function POST(req: Request) {
  try {
    ...
    if (!MercadoPagoBillingAdapter.validateSignature(signatureHeader)) {
      return NextResponse.json({ error: 'UNAUTHORIZED: Assinatura do Mercado Pago inválida.' }, { status: 401 });
    }

    const payload = await req.json();
    const canonicalEvent = MercadoPagoBillingAdapter.parseEvent(req.headers, payload);
